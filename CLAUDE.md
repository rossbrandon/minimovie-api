...
- **Metrics**: `metrics.M` is a package global that records into the SDK's no-op provider until `metrics.Init` installs the exporter, so it is never nil and never guarded. Stores time queries with `defer metrics.TrackDbDuration(ctx, "entity.operation")()`: the count is labelled by the whole name, the duration histogram only by the entity before the dot. Grafana Cloud's free tier caps active series, so a new label must have a small, fixed set of values, and a new histogram needs a reason; `grafana.json` in the repo root is the dashboard export.
